...
# ------------------------------------------------------------------------------
def vatmos1(np.ndarray[DTYPE_t, dim=1] h):
    cdef Py_ssize_t ntraf = h.shape[0]
    cdef np.ndarray[DTYPE_t, ndim=1)] p = np.empty(ntraf, DTYPE_t)
    cdef np.ndarray[DTYPE_t, ndim=1)] rho = np.empty(ntraf, DTYPE_t)
    cdef np.ndarray[DTYPE_t, ndim=1)] T = np.empty(ntraf, DTYPE_t)

    cdef Py_ssize_t ii
    cdef double rhotrop, dhstrat
    for ii in range(ntraf):
        # Temp
        T[ii] = temp(h[ii])

        # Density
        rhotrop = rho0 * (T[ii] / T0) ** 4.256848030018761
        dhstrat = max(0.0, h - 11000.0)
        rho[ii] = rhotrop * exp(-dhstrat / 6341.552161)
