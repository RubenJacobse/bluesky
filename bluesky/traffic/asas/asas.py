...
class ASAS(TrafficArrays):
    ...
    def SetResooff(self, ac_reso_off=""):
        ...
        ac_ids = (ac_reso_off.split(',') if len(ac_reso_off.split(',')) > 1
                  else ac_reso_off.split(' '))
        ...
        if set(ac_ids) <= set(self.resoofflst):
            self.resoofflst = [x for x in self.resoofflst if x not in set(ac_ids)]
        else:
            self.resoofflst.extend(ac_ids)
