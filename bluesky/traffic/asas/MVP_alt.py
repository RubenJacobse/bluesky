...
def resolve(asas, traf):
    ...
    mvp_gsnorth = np.zeros(traf.ntraf, dtype=np.float32)
    mvp_gseast = np.zeros(traf.ntraf, dtype=np.float32)
    ...
    mvp_trk = new_track
    mvp_tas = new_gs_capped
    mvp_vs = new_vs_capped

    # Stores resolution vector
    mvp_gseast[idxs] = mvp_tas[idxs] * np.radians(np.sin(mvp_trk[idxs]))
    mvp_gsnorth[idxs] = mvp_tas[idxs] * np.radians(np.cos(mvp_trk[idxs]))
    ...
    # altitude.
    signdvs = np.sign(mvp_vs - traf.ap.vs * np.sign(traf.selalt - traf.alt))
    signalt = np.sign(asas.alt - traf.selalt)
    mvp_alt = np.where(np.logical_or(signdvs == 0, signdvs == signalt), asas.alt, traf.selalt)
    ...
    asasalttemp = mvp_vs * t_solve_vertical + traf.alt
    mvp_alt[altCondition] = asasalttemp[altCondition]
    ...
    mvp_alt = mvp_alt*(1-asas.swresohoriz) + traf.selalt*asas.swresohoriz

    return mvp_trk, mvp_tas, mvp_vs, mvp_gseast, mvp_gsnorth, mvp_alt
