...
def init(cfgfile=""):
    ...
    then create an initial config file.
    """
    ...
    # adjust the startup path and change the path of configurable files
    # to $home/bluesky
    if getattr(sys, "frozen", False):
        srcdir = os.path.dirname(sys.executable)
        rundir = os.path.join(os.path.expanduser("~"), "bluesky")

    # If BlueSky is installed as a python package the location of the data
    # files need to be adjusted so that importing will not fail when copying
    # config file below
